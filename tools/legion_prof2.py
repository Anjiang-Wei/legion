...
class State(object):
    ...
    @typecheck
    def show_copy_matrix2(self, output_prefix: str) -> None:
        template_file_name = os.path.join(root_dir, "legion_prof_copy.html.template")
        tsv_file_name = output_prefix + ".tsv"
        html_file_name = output_prefix + ".html"
        print('Generating copy visualization files %s and %s' % (tsv_file_name,html_file_name))
        ...
        def node_id(memory: Memory) -> int:
            return (memory.mem_id >> 39) & ((1 << 5) - 1)
        memories = sorted(self.memories.values())
        # print("memories", memories)

        tsv_file = open(tsv_file_name, "w")
        tsv_file.write("source\ttarget\ttotal_size\n")
        total_size = 0
        inter_size = 0
        intra_size = 0
        for i in range(0, len(memories)):
            for j in range(0, len(memories)):
                ...
                dst = memories[j]
                # framebuffer memory
                if src.kind == memory_kinds[6] and dst.kind == memory_kinds[6] and src.mem_id != dst.mem_id:
                    channel = self.find_channel(src, dst)
                    for copy in channel.copies:
                        total_size += copy.size
                    if src.node_id != dst.node_id:
                        for copy in channel.copies:
                            inter_size += copy.size
                            # print(f"{src.node_id} to {dst.node_id} {copy.size / 1024}")
                    if src.node_id == dst.node_id:
                        for copy in channel.copies:
                            intra_size += copy.size
                            # print(f"{src.node_id} to {dst.node_id} {copy.size / 1024}")
        print("all %.2f\ninter %.2f\nintra %.2f\n" % (total_size / 1024 / 1024, inter_size / 1024 / 1024, intra_size / 1024 / 1024))
        tsv_file.write("all %.2f\ninter%.2f\nintra%.2f\n" % (total_size / 1024 / 1024, inter_size / 1024 / 1024, intra_size / 1024 / 1024))
        tsv_file.close()
...
def main() -> None:
    ...
    if print_stats:
        ...
        if show_copy_matrix:
            state.show_copy_matrix2(copy_output_prefix)
